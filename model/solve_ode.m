function [Vnet,conc,Er_final] = solve_ode(expanded_model,t_interval,y0,kinetic_param,conv_limit)
...
try
  ...
  eval(strcat('options=odeset(''Jacobian'',@(t,y)jacobFun_',expanded_model.model_name,...
      '(t,y,kinetic_param),''Events'',@(t,y)event_function(t,y,a,kinetic_param,expanded_model,conv_limit));')); 
  ...
  Vnet_perturb_ss=get_Vnet(expanded_model,0,Y(end,:),kinetic_param);
  Er_final=max(abs(expanded_model.unexpanded_model.S*Vnet_perturb_ss(1:size(expanded_model.unexpanded_model.S,2))));
...
clearvars -except Vnet conc Er_final
conc=real(conc);
Vnet=real(Vnet);
end
