...
function [Vnet,concentration_perturb,Error]=Main_Module(x1,x2,growth_condition,conv_limit)
rng('default');
rng(1);
...
t_interval = [0:1e4];% Change it accordingly
%t_interval = [0:1:1e2];
warning off
...
fprintf('\n-------------------------------\n');
       ...
       % Integration of ODEs
       [Vnet,concentration_perturb,Error] = solve_ode(network_data,t_interval,y0,elementary_kinetic_parameter,conv_limit);
